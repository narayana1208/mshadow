...
private:
  ...
  static CuBlasHandle *inst_;
  cublasHandle_t handle_;
public:
  static CuBlasHandle *AccessObj() {
    if (!inst_) inst_ = new CuBlasHandle();
    return inst_;
  }
  static void Init() {
    cublasStatus_t stat;
    stat = cublasCreate(&(AccessObj()->handle_));
    utils::Assert(stat == CUBLAS_STATUS_SUCCESS, "CUBLAS Init Error");
  }
  static void Shutdown() {
    cublasDestroy(AccessObj()->handle_);
  }
  static cublasHandle_t &Get() {
    return AccessObj()->handle_;
  }
};
CuBlasHandle *CuBlasHandle::inst_ = NULL;
